...
addEventListener('message', ({ data }: { data: WebWorkerRequest }) => {
  if (data.type !== 'regenerate') {
    return;
  }

  const { shadesString, id } = data;
  const shades = shadesString.map((shadeString: string) =>
    Shade.parse(shadeString)
  );

  const newShades = regenerateShades(shades);

  const message: RegenerateResponse = {
    id,
    type: 'regenerate',
    shadesString: newShades.map((shade) => shade.toString()),
  };
  postMessage(message);
});
 ...
 * @param color
 */
export default function regenerateShades(shades: Array<Shade>): Array<Shade> {
  ...
  indices[0] = 50;
  distributeIndices(shades, indices);

  // Calculate minimal saturation
  const minimum = calculateMinSaturation(shades);

  // Create "white"
  const white = createBorderShade(shades[0], minimum, true);
  shades.unshift(white);

  // Create "black"
  const black = createBorderShade(shades[shades.length - 1], minimum, false);
  shades.push(black);
  ...
  for (const index of indices) {
    ...
    const saturation = calculateSaturation(luminosity, lighter, darker, third);
 ...
 * @param indices
 */
function distributeIndices(shades: Array<Shade>, indices: number[]): void {
  ...
  offsetIndicesFromLightest(shades);
}
 ...
 * @param shades
 */
function offsetIndicesFromLightest(shades: Shade[]): void {
  let minIndex = 0;
  for (let i = 0; i < shades.length; i++) {
    ...
    if (minIndex >= shade.index) {
      if (minIndex < 900) {
        ...
        shade.index = 900;
        offsetIndicesFromDarkest(shades, i);
      }
    }
 ...
 * @param start
 */
function offsetIndicesFromDarkest(shades: Shade[], start: number): void {
 ...
 * @param shades
 */
function calculateMinSaturation(shades: Shade[]): {
  saturation: number;
  brightness: number;
} {
...
function createBorderShade(
  ...
  const hue = calculateHue(shade, white);
  const saturation = calculateMaxSaturation(minimum, shade, index);
 ...
 * @param white
 */
function calculateHue(shade: Shade, white: boolean) {
 ...
 * @param index
 */
function calculateMaxSaturation(
  minimum: { saturation: number; brightness: number },
  neighbor: Shade,
  index: number
) {
